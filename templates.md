...
如果你已经有一条候选任务，并且能说明业务 Owner、允许输入和人工接管，第一次只需要完成下列三项，来决定能否进入第一轮**受控测试设计**。它们不是新的模板，也不是对外、写入、敏感数据、跨系统工具或多人运行的授权。

| 顺序 | 先完成什么 | 使用的已有资产 | 完成后你应能回答 |
| --- | --- | --- | --- |
| 1 | 用例立项 | [用例立项卡](./17-附录A-可直接复用的清单与模板.md#asset-use-case-charter) | 为什么做、范围多小、谁负责、什么结果不能接受、何时停止？ |
| 2 | 任务定义 | [任务定义模板](./17-附录A-可直接复用的清单与模板.md#asset-task-definition) | 什么触发、允许什么输入、要交付什么、怎样算合格、人工保留什么判断？ |
| 3 | 验证与回退 | [通用验证清单](./17-附录A-可直接复用的清单与模板.md#asset-verification-checklist) + 任务定义中的“人工控制与回退”字段 | 谁审核、依据什么、失败后回到哪条人工路径、缺什么就停止？ |

若这三项中任何一项无法填写，不要用更多模板填补缺口：记录阻塞条件并回到[第 2 章自诊断](/handbook/diagnose-your-constraint)。完成 L0 后，只有在确实开始受控测试时，才按下方启动套件继续使用提示词、周度记录和问题升级资产；涉及高风险条件时，直接按阶段×风险资产地图补齐相应控制。
...
| 已选任务，想跑第一个受控闭环 | 先完成 L0 三项；进入受控测试时再固定提示词、记录失败并建立升级路径。 | [L0 三项](#l0第一次只用三项已有资产) · [任务定义](./17-附录A-可直接复用的清单与模板.md#asset-task-definition) · [验证清单](./17-附录A-可直接复用的清单与模板.md#asset-verification-checklist) |
